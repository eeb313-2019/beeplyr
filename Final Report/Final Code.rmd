...
Land Use and Hymenoptera Family Diveristy Analysis
```{r}
#Checking Assumptions of Normality
pol.hymenoptera %>% 
  ggplot(aes(x = diversity))+ geom_histogram()
    # as seen by the graoh, it does not graphically appear that the data is normally distributed

#Shapiro-Wilk Test
shapiro.test(pol.hymenoptera$diversity)
...
leveneTest(diversity~land_use, data=pol.hymenoptera)
...
```{r}
#ANOVA 
anova(lm(diversity~land_use, data=pol.hymenoptera))
    #p.value is 3.1e-16

anova(lm(diversity~land_use, data=pol.hymenoptera.andrenidea))
...
summary(model1)
    # it seems only the research plot, wildlife refuge, and intercept are significant. Total significant p value tho. 
```
...
ggplot() +
  geom_polygon(data=texas_only, aes(x=long, y=lat, group=group), colour="black", fill = "grey32") + 
  geom_tile(data=raster_df.filtered.tx, aes(x=x,y=y,fill=w001001))+
...
ggplot() +
  geom_polygon(data=nd_only, aes(x=long, y=lat, group=group), colour="black", fill = "grey32") + 
  geom_tile(data=raster_df.filtered.nd, aes(x=x,y=y,fill=w001001), show.legend = FALSE)+
